 ...
 */
export const listCategoriesService = async (targetUserId: TargetParams): Promise<serviceResponse> => {
    try {
        const listCategories = await prisma.user.findUnique({
            ...
            select: {
                id: true,
                username: true,
                categories: {  // nested select to fetch user's categories
                    select: {
                        id: true,
                        category_name: true,
                        slug: true,
...
export const createCategoriesService = async (
    ...
    categoryName: CategoryNameParams
 ...
 */
export const categoryDetailsService = async (targetCategoryId: TargetParams): Promise<serviceResponse> => {
    try {
        const success = await prisma.category.findFirst({
            ...
            select: {
                id: true,
                category_name: true,
                user: {  // nested select to include creator's info
                    select: {
                        id: true,
                        username: true
                    }
 ...
 * @returns {Promise<serviceResponse>} - Success status and message
 */
export const deleteCategoryService = async (targetCategoryId: TargetParams): Promise<serviceResponse> => {
    try {
        const success = await prisma.category.update({
            where: { id: targetCategoryId },
            data: {
