 ...
 */
export const toBaseSlug = (nameParts) => {
    ...
    const parts = Array.isArray(nameParts) ? nameParts : [nameParts];
    // Flatten and clean parts, remove duplicate words
    const base = [...new Set(parts.flatMap((item) => clean(item).split(' ')) // split cleaned string by spaces
        )];
 ...
 */
export const generateUniqueSlug = async (nameParts) => {
