...
export const signUpService = async (reqBody) => {
    try {
        ...
        if (!userRole)
            return { success: false, message: 'User role not granted' };
        ...
        if (!userPermissions)
            return { success: false, message: 'User permissions not granted' };
 ...
 */
export const userDetailsService = async (tokenUser) => {
    try {
        const userExists = await prisma.user.findUnique({
