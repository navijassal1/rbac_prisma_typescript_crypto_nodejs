...
userPublicRouter.post("/signup", signupValidation, validateResult, signUp);
// User login route
// 1. loginValidation → validates request body
// 2. validateResult → checks validation errors
// 3. login → handles authentication logic
userPublicRouter.post("/login", loginValidation, validateResult, login);
// Refresh token route
...
userPublicRouter.get("/refresh-token", refreshTokenValidation, validateResult, refreshAccessToken);
// Export router to be used in the main app
export default userPublicRouter;
//# sourceMappingURL=user.public.routes.js.map